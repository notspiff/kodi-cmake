...
void CVideoDatabase::CreateTables()
{
  ...
  CLog::Log(LOGINFO, "create import_link table");
...
void CVideoDatabase::CreateViews()
{
  ...
  std::string episodeview = PrepareSQL("CREATE VIEW episode_view AS SELECT "
                                      ...
                                      "  LEFT JOIN import ON"
                                      "    import.import_id = import_link.import_id AND import.media_type = 'episode'"
                                      "  LEFT JOIN source ON"
  ...
  std::string tvshowview = PrepareSQL("CREATE VIEW tvshow_view AS SELECT "
                                     ...
                                     "  LEFT JOIN import ON"
                                     "    import.import_id = import_link.import_id AND import.media_type = 'tvshow'"
                                     "  LEFT JOIN source ON"
                                     "    source.source_id = import.source_id "
                                     "  LEFT JOIN path AS importsPath ON"
                                     "    importsPath.idPath = import.idPath "
  ...
  std::string seasonview = PrepareSQL("CREATE VIEW season_view AS SELECT "
                                     ...
                                     "  LEFT JOIN import ON"
                                     "    import.import_id = import_link.import_id AND import.media_type = 'season'"
                                     "  LEFT JOIN source ON"
                                     "    source.source_id = import.source_id"
                                     "  LEFT JOIN path AS importsPath ON"
                                     "    importsPath.idPath = import.idPath "
  ...
  m_pDS->exec("CREATE VIEW musicvideo_view AS SELECT"
              ...
              "  LEFT JOIN import ON"
              "    import.import_id = import_link.import_id AND import.media_type = 'musicvideo'"
              "  LEFT JOIN source ON"
              "    source.source_id = import.source_id"
              "  LEFT JOIN path AS importsPath ON"
              "    importsPath.idPath = import.idPath");
  ...
  m_pDS->exec("CREATE VIEW movie_view AS SELECT"
              ...
              "  LEFT JOIN import ON"
              "    import.import_id = import_link.import_id AND import.media_type = 'movie'"
              "  LEFT JOIN source ON"
              "    source.source_id = import.source_id"
              "  LEFT JOIN path AS importsPath ON"
              "    importsPath.idPath = import.idPath");
...
void CVideoDatabase::UpdateFileDateAdded(int idFile, const std::string& strFileNameAndPath, CDateTime dateAdded /* = CDateTime() */)
{
...
int CVideoDatabase::GetImportId(int idPath, const MediaType& mediaType, int idSource /* = -1 */)
{
  try
  {
    ...
    std::string strSQL = PrepareSQL("SELECT import_id FROM import WHERE idPath = %d AND media_type = '%s'", idPath, mediaType.c_str());
    if (idSource > 0)
...
//********************************************************************************************************************************
int CVideoDatabase::AddMovie(const std::string& strFilenameAndPath, CDateTime dateAdded /* = CDateTime() */)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
...
bool CVideoDatabase::AddPathToTvShow(int idShow, const std::string &path, const std::string &parentPath, CDateTime dateAdded /* = CDateTime() */)
{
...
//********************************************************************************************************************************
int CVideoDatabase::AddEpisode(int idShow, const std::string& strFilenameAndPath, CDateTime dateAdded /* = CDatetime() */)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
...
std::vector<CMediaImportSource> CVideoDatabase::GetSources()
{
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      std::set<MediaType> media_types(vecMediaTypes.begin(), vecMediaTypes.end());
...
bool CVideoDatabase::SetImportForItem(int idMedia, const CMediaImport &import)
{
  if (idMedia <= 0 ||
      import.GetMediaType().empty())
    return false;
  ...
  try
  {
    ...
    sql = PrepareSQL("SELECT 1 FROM import_link WHERE import_id = %d AND media_id = %d AND media_type = '%s'", idImport, idMedia, import.GetMediaType().c_str());
    m_pDS->query(sql.c_str());
    if (m_pDS->num_rows() > 0)
      return true;

    sql = PrepareSQL("INSERT INTO import_link (import_id, media_id, media_type) VALUES (%d, %d, '%s')", idImport, idMedia, import.GetMediaType().c_str());
    m_pDS->exec(sql.c_str());
    return true;
  }
  catch (...)
...
bool CVideoDatabase::RemoveImportFromItem(int idMedia, const CMediaImport& import)
{
  if (idMedia <= 0 ||
    import.GetMediaType().empty())
    return false;
  ...
  try
  {
    ...
    sql = PrepareSQL("DELETE FROM import_link WHERE import_id = %d AND media_id = %d AND media_type = '%s'", idImport, idMedia, import.GetMediaType().c_str());
    m_pDS->exec(sql.c_str());
    return true;
  }
  catch (...)
...
bool CVideoDatabase::DeleteItemsFromImport(const CMediaImport& import)
{
  ...
  try
  {
    ...
    bool isContainer = MediaTypes::IsContainer(mediaType);
    std::string table = mediaType;
    std::string idColumn;
    if (MediaTypes::IsMediaType(mediaType, MediaTypeMovie))
      idColumn = "idMovie";
    else if (MediaTypes::IsMediaType(mediaType, MediaTypeEpisode))
      idColumn = "idEpisode";
    else if (MediaTypes::IsMediaType(mediaType, MediaTypeMusicVideo))
      idColumn = "idMVideo";
    else if (MediaTypes::IsMediaType(mediaType, MediaTypeTvShow))
      idColumn = "idShow";
    else if (MediaTypes::IsMediaType(mediaType, MediaTypeSeason))
    {
      idColumn = "idSeason";
      table = "seasons";
    }
    ...
    // containers don't have files and they may belong to multiple imports
    if (!isContainer)
    {
...
void CVideoDatabase::SetImportItemsEnabled(bool enabled, const CMediaImport &import)
{
  ...
  try
  {
    ...
    if (!import.GetMediaType().empty())
      sql += PrepareSQL(" AND import_link.media_type = '%s'", import.GetMediaType().c_str());
