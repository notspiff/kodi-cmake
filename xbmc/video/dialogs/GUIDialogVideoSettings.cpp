...
void CGUIDialogVideoSettings::OnSettingChanged(const CSetting *setting)
{
  ...
  if (settingId == SETTING_VIDEO_VIEW_MODE)
  {
  ...
  else if (!m_viewModeChanged &&
          (settingId == SETTING_VIDEO_ZOOM ||
...
void CGUIDialogVideoSettings::InitializeSettings()
{
  ...
  if (!entries.empty())
    AddSpinner(groupVideo, SETTING_VIDEO_DEINTERLACEMODE, 16037, 0, (int*)&videoSettings.m_DeinterlaceMode, entries);
  ...
  if (!entries.empty())
  {
    CSettingInt *settingInterlaceMethod = AddSpinner(groupVideo, SETTING_VIDEO_INTERLACEMETHOD, 16038, 0, (int*)&videoSettings.m_InterlaceMethod, entries);
...
//  entries.push_back(make_pair(?????, VS_SCALINGMETHOD_NEDI));
  ...
  AddSpinner(groupVideo, SETTING_VIDEO_SCALINGMETHOD, 16300, 0, (int*)&videoSettings.m_ScalingMethod, entries);
