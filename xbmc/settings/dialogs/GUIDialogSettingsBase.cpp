...
void CGUIDialogSettingsBase::SetupControls(bool createSettings /* = true */)
{
  ...
  m_pOriginalLabel = dynamic_cast<CGUILabelControl *>(GetControl(CONTROL_DEFAULT_LABEL));
...
std::set<std::string> CGUIDialogSettingsBase::CreateSettings()
{
  ...
  for (SettingGroupList::const_iterator groupIt = groups.begin(); groupIt != groups.end(); ++groupIt)
  {
    ...
    bool hideSeparator = (*groupIt)->HideSeparator();
    bool groupLabelBeforeSeparator = (*groupIt)->LabelBeforeSeparator();
    if (first)
      first = false;
    else if (!groupLabelBeforeSeparator && !hideSeparator)
      AddSeparator(group->GetWidth(), iControlID);

    if ((*groupIt)->GetLabel() > 0)
      AddLabel(group->GetWidth(), iControlID, (*groupIt)->GetLabel());

    if (groupLabelBeforeSeparator && !hideSeparator)
      AddSeparator(group->GetWidth(), iControlID);
...
{
  if (m_pOriginalLabel == NULL)
    return NULL;

  CGUIControl *pControl = new CGUILabelControl(*m_pOriginalLabel);
  if (pControl == NULL)
    return NULL;
