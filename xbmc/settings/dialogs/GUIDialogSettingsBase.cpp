...
bool CGUIDialogSettingsBase::OnMessage(CGUIMessage &message)
{
  switch (message.GetMessage())
  {
    ...
    case GUI_MSG_UPDATE_ITEM:
    {
      ...
      else if (message.GetControlId() >= CONTROL_SETTINGS_START_CONTROL && message.GetControlId() < (int)(CONTROL_SETTINGS_START_CONTROL + m_settingControls.size()))
      {
    ...
    case GUI_MSG_SETFOCUS:
    {
      ...
      // in case the initially focused control is disabled we need to find
      // another control that is not disabled which will get the focus
      int controlId = message.GetControlId();
      if (controlId >= CONTROL_SETTINGS_START_CONTROL && controlId < static_cast<int>(CONTROL_SETTINGS_START_CONTROL + m_settingControls.size()))
      {
        BaseSettingControlPtr settingControl = GetSettingControl(controlId);
        if (settingControl == NULL || settingControl->GetControl() == NULL ||
            settingControl->GetControl()->IsDisabled())
        {
          std::vector<BaseSettingControlPtr>::const_iterator focusedControl = m_settingControls.begin() + (controlId - CONTROL_SETTINGS_START_CONTROL);
          std::vector<BaseSettingControlPtr>::const_iterator control = focusedControl + 1;
          if (control == m_settingControls.end())
            control = m_settingControls.begin();
          while (control != focusedControl)
          {
            if ((*control) != NULL && (*control)->GetControl() != NULL &&
                !(*control)->GetControl()->IsDisabled())
            {
...
void CGUIDialogSettingsBase::SetupControls(bool createSettings /* = true */)
{
  ...
  m_defaultControl = m_pOriginalCategoryButton != NULL ? CONTROL_SETTINGS_START_BUTTONS : CONTROL_SETTINGS_START_CONTROL;
}
