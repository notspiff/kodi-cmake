...
void CGUIDialogAudioDSPSettings::InitializeSettings()
{
   ...
   */
  {
    ...
    AddInfoStringButton(group, SETTING_STREAM_INFO_INPUT_SAMPLERATE, 15092, 0, m_InputSamplerate);
    ...
    AddInfoStringButton(group, SETTING_STREAM_INFO_OUTPUT_SAMPLERATE, 15092, 0, m_OutputSamplerate);
    ...
    m_CPUUsage = StringUtils::Format("%.02f %%", m_ActiveStreamProcess->GetCPUUsage());
    AddInfoStringButton(group, SETTING_STREAM_INFO_CPU_USAGE, 15094, 0, m_CPUUsage);
      ...
      if (CActiveAEDSP::Get().GetAudioDSPAddon(m_ActiveModes[i]->AddonID(), addon))
      {
        ...
        switch (m_ActiveModes[i]->ModeType())
        {
          case AE_DSP_MODE_TYPE_INPUT_RESAMPLE:
            group = AddGroup(category, 15087, -1, true);
          ...
          case AE_DSP_MODE_TYPE_OUTPUT_RESAMPLE:
            group = AddGroup(category, 15088, -1, true);
          ...
          case AE_DSP_MODE_TYPE_MASTER_PROCESS:
            group = AddGroup(category, 15084, -1, true);
          ...
          case AE_DSP_MODE_TYPE_PRE_PROCESS:
            if (!foundPreProcess)
            {
              ...
              group = AddGroup(category, 15085, -1, true);
            }
            label = addon->GetString(m_ActiveModes[i]->ModeName());
            break;
          case AE_DSP_MODE_TYPE_POST_PROCESS:
            if (!foundPostProcess)
            {
              ...
              group = AddGroup(category, 15086, -1, true);
            }
            label = addon->GetString(m_ActiveModes[i]->ModeName());
            break;
          default:
        ...
        string settingId = StringUtils::Format("%s%i", SETTING_STREAM_INFO_MODE_CPU_USAGE, i);
        AddInfoStringButton(group, settingId, 15041, 0, m_ActiveModesData[i].CPUUsage);
      }
    }
  }
}
