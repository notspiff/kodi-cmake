...
enum InputPrimitiveType
{
  InputPrimitiveTypeUnknown = 0,
  InputPrimitiveTypeButton,
  InputPrimitiveTypeHatDirection,
  InputPrimitiveTypeSemiAxis,
};
...
public:
   ...
   */
  CInputPrimitive(void);

  /*!
   * \brief Construct an input primitive representing a button press
   */
  CInputPrimitive(unsigned int buttonIndex);

  /*!
   * \brief Construct an input primitive representing one of the four direction
   *        arrows on a dpad
   */
  CInputPrimitive(unsigned int hatIndex, HatDirection direction);

  /*!
   * \brief Construct an input primitive representing the positive or negative
   *        half of an axis
   */
  CInputPrimitive(unsigned int axisIndex, SemiAxisDirection direction);

  bool operator==(const CInputPrimitive& rhs) const;
  bool operator<(const CInputPrimitive& rhs) const;

  bool operator!=(const CInputPrimitive& rhs) const { return !operator==(rhs); }
  bool operator>(const CInputPrimitive& rhs) const  { return !(operator<(rhs) || operator==(rhs)); }
  bool operator<=(const CInputPrimitive& rhs) const { return   operator<(rhs) || operator==(rhs); }
  bool operator>=(const CInputPrimitive& rhs) const { return  !operator<(rhs); }

  InputPrimitiveType Type(void) const        { return m_type; }
