...
bool CSeasonImportHandler::HandleImportedItems(CVideoDatabase &videodb, const CMediaImport &import, const CFileItemList &items, IMediaImportTask *task)
{
  ...
  task->SetProgressTitle(StringUtils::Format(g_localizeStrings.Get(37064), MediaTypes::GetPluralLocalization(MediaTypeSeason).c_str(), import.GetSource().GetFriendlyName().c_str()));
