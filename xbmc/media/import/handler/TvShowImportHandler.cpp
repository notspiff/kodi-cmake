...
bool CTvShowImportHandler::HandleImportedItems(CVideoDatabase &videodb, const CMediaImport &import, const CFileItemList &items, IMediaImportTask *task)
{
  ...
  task->SetProgressTitle(StringUtils::Format(g_localizeStrings.Get(37026), MediaTypes::GetPluralLocalization(MediaTypeTvShow).c_str(), import.GetSource().GetFriendlyName().c_str()));
