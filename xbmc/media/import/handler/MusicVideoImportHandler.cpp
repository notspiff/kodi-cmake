...
bool CMusicVideoImportHandler::HandleImportedItems(CVideoDatabase &videodb, const CMediaImport &import, const CFileItemList &items, IMediaImportTask *task)
{
  ...
  task->SetProgressTitle(StringUtils::Format(g_localizeStrings.Get(37026), MediaTypes::GetPluralLocalization(MediaTypeMusicVideo).c_str(), import.GetSource().GetFriendlyName().c_str()));
  ...
  for (int i = 0; i < newItems.Size(); i++)
  {
    ...
    videodb.SetDetailsForMusicVideo(pItem->GetPath(), *(pItem->GetVideoInfoTag()), pItem->GetArt());
