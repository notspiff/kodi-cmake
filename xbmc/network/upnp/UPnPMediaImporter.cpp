...
static bool Search(CMediaImportRetrievalTask *task, PLT_DeviceDataReference &device, const MediaType &mediaType, std::vector<CFileItemPtr> &items)
{
  ...
  task->GetProgressBarHandle()->SetText(StringUtils::Format(g_localizeStrings.Get(39018).c_str(), MediaTypes::GetPluralLocalization(supportedMediaType->mediaType).c_str()));
