...
extern "C"
{
  ...
  struct VFSEntry
  {
    ...
    //! \brief Switch to the next channel (LiveTV streams)
    //! \param ctx The context of the file
    //! \param preview Whether or not this is a channel preview
    //! \return True if channel switch was successful, false otherwise
    //! \sa ILiveTVInterface::NextChannel
    bool (__cdecl* NextChannel)   (void* ctx, bool preview);

    //! \brief Switch to the previous channel (LiveTV streams)
    //! \param ctx The context of the file
    //! \param preview Whether or not this is a channel preview
    //! \return True if channel switch was successful, false otherwise
    //! \sa ILiveTVInterface::PrevChannel
    bool (__cdecl* PrevChannel)   (void* ctx, bool preview);

    //! \brief Switch to a given channel (LiveTV streams)
    //! \param ctx The context of the file
    //! \param channel The channel to switch to
    //! \return True if channel switch was successful, false otherwise
    //! \sa ILiveTVInterface::SelectChannel
    bool (__cdecl* SelectChannel) (void* ctx, unsigned int channel);

    //! \brief Get total time of stream
    //! \param ctx The context of the file
    //! \return Total time for stream
    //! \sa ILiveTVInterface::GetTotalTime
    int  (__cdecl* GetTotalTime)  (void* ctx);

    //! \brief Get start time of stream
    //! \param ctx The context of the file
    //! \return Start time for stream
    //! \sa ILiveTVInterface::GetTotalTime
    int  (__cdecl* GetStartTime)  (void* ctx);

    //! \brief Update information for item
    //! \param ctx The context of the file
    //! \return Whether or not info was updated
    //! \sa ILiveTVInterface::UpdateItem
