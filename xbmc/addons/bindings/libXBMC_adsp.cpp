 ...
 : m_Filename(filename)
{
  m_PlayHandle = NULL;
  if (!hdl || !cb)
    fprintf(stderr, "libXBMC_adsp-ERROR: ADSP_get_sound_play is called with NULL handle !!!\n");
  else
  {
    m_Handle     = hdl;
    m_cb         = cb;
    ...
    if (!m_PlayHandle)
      fprintf(stderr, "libXBMC_adsp-ERROR: ADSP_get_sound_play can't get callback table from XBMC !!!\n");
  }
}
