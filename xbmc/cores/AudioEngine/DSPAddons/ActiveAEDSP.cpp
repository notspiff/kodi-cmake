...
bool CActiveAEDSP::TranslateCharInfo(DWORD dwInfo, std::string &strValue) const
{
  ...
  case ADSP_ACTIVE_STREAM_TYPE:
    strValue = g_localizeStrings.Get(m_usedProcesses[m_activeProcessId]->GetUsedStreamType()+15001);
    break;
  case ADSP_DETECTED_STREAM_TYPE:
    strValue = g_localizeStrings.Get(m_usedProcesses[m_activeProcessId]->GetDetectedStreamType()+15001);
    break;
...
bool CActiveAEDSP::UpdateAddons(void)
{
  ...
  for (unsigned iAddonPtr = 0; iAddonPtr < m_Addons.size(); iAddonPtr++)
  {
    ...
    {
      CAddonMgr::Get().DisableAddon(dspAddon->ID(), true);
      usableAddons--;
    }
  }

  if ((!bReturn || usableAddons == 0) && !m_bNoAddonWarningDisplayed &&
...
//@{
enum AEChannel CActiveAEDSP::GetXBMCChannel(AE_DSP_CHANNEL channel)
{
  switch (channel)
  {
