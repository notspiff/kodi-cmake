...
void CGUIDialogMediaFilter::InitializeSettings()
{
  ...
  for (unsigned int index = 0; index < NUM_FILTERS; index++)
  {
    ...
    if (filter.controlType == "edit")
    {
      ...
      {
        std::string *pData = new std::string(data.asString());
        filter.data = static_cast<void*>(pData);
        filter.setting = AddEdit(group, settingId, filter.label, 0, pData, true, false, filter.label, true);
      }
      else if (filter.settingType == SettingTypeInteger)
      {
        int *pData = new int(static_cast<int>(data.asInteger()));
        filter.data = static_cast<void*>(pData);
        filter.setting = AddEdit(group, settingId, filter.label, 0, pData, 0, 1, 0, false, filter.label, true);
      }
      else if (filter.settingType == SettingTypeNumber)
      {
        float *pData = new float(data.asFloat());
        filter.data = static_cast<void*>(pData);
        filter.setting = AddEdit(group, settingId, filter.label, 0, pData, 0.0f, 1.0f, 0.0f, false, filter.label, true);
      }
    }
    else if (filter.controlType == "toggle")
    {
      int *pData = new int(CHECK_ALL);
      if (filter.rule != NULL)
        *pData = filter.rule->m_operator == CDatabaseQueryRule::OPERATOR_TRUE ? CHECK_YES : CHECK_NO;
      ...
      filter.data = static_cast<void*>(pData);
      filter.setting = AddSpinner(group, settingId, filter.label, 0, pData, entries, true);
    }
    else if (filter.controlType == "list")
    {
    ...
    else if (filter.controlType == "range")
    {
      ...
      if (filter.settingType == SettingTypeInteger)
      {
        ...
        values[0] = valueLower.isNull() ? min : static_cast<int>(valueLower.asInteger());
        values[1] = valueUpper.isNull() ? max : static_cast<int>(valueUpper.asInteger());
        filter.data = static_cast<void*>(values);

        if (filter.controlFormat == "integer")
          filter.setting = AddRange(group, settingId, filter.label, 0, &values[0], &values[1], min, interval, max, -1, 21469, true);
        else if (filter.controlFormat == "percentage")
          filter.setting = AddPercentageRange(group, settingId, filter.label, 0, &values[0], &values[1], -1, 1, 21469, true);
        else if (filter.controlFormat == "date")
          filter.setting = AddDateRange(group, settingId, filter.label, 0, &values[0], &values[1], min, interval, max, -1, 21469, true);
        else if (filter.controlFormat == "time")
          filter.setting = AddTimeRange(group, settingId, filter.label, 0, &values[0], &values[1], min, interval, max, -1, 21469, true);
      }
      else if (filter.settingType == SettingTypeNumber)
      {
        ...
        values[0] = valueLower.isNull() ? min : valueLower.asFloat();
        values[1] = valueUpper.isNull() ? max : valueUpper.asFloat();
        filter.data = static_cast<void*>(values);
...
void CGUIDialogMediaFilter::Reset(bool filtersOnly /* = false */)
{
  ...
  // delete all data pointers of the existing filters
  for (std::map<std::string, Filter>::const_iterator itFilter = m_filters.begin(); itFilter != m_filters.end(); ++itFilter)
  {
    if (itFilter->second.data == NULL)
      continue;

    switch (itFilter->second.settingType)
    {
      case SettingTypeBool:
        delete static_cast<bool*>(itFilter->second.data);
        break;

      case SettingTypeInteger:
        delete static_cast<int*>(itFilter->second.data);
        break;

      case SettingTypeNumber:
        delete static_cast<float*>(itFilter->second.data);
        break;

      case SettingTypeString:
        delete static_cast<std::string*>(itFilter->second.data);
        break;

      case SettingTypeList:
      {
        CSettingList *settingList = static_cast<CSettingList*>(itFilter->second.setting);
        if (settingList == NULL)
          break;

        bool isArray = itFilter->second.controlType == "range";
        switch (settingList->GetElementType())
        {
          case SettingTypeBool:
            if (isArray)
              ...
              delete static_cast<std::vector<bool>*>(itFilter->second.data);
            break;
          ...
          case SettingTypeInteger:
            if (isArray)
              ...
              delete static_cast<std::vector<int>*>(itFilter->second.data);
            break;
          ...
          case SettingTypeNumber:
            if (isArray)
              ...
              delete static_cast<std::vector<float>*>(itFilter->second.data);
            break;
          ...
          case SettingTypeString:
            if (isArray)
              ...
              delete static_cast<std::vector<std::string>*>(itFilter->second.data);
            break;
...
void CGUIDialogMediaFilter::GetRange(const Filter &filter, int &min, int &interval, int &max)
{
  ...
  else if (filter.field == FieldYear)
  {
    ...
    if (m_mediaType == MediaTypeMovie || m_mediaType == MediaTypeTvShow || m_mediaType == MediaTypeEpisode)
    {
      CStdString table;
      CStdString year;
      if (m_mediaType == MediaTypeMovie)
      {
        table = "movieview";
        year = DatabaseUtils::GetField(FieldYear, MediaTypeMovie, DatabaseQueryPartWhere);
      }
      else if (m_mediaType == MediaTypeTvShow)
      {
        table = "tvshowview";
    ...
    else if (m_mediaType == MediaTypeAlbum || m_mediaType == MediaTypeSong)
    {
      CStdString table;
  ...
  else if (filter.field == FieldAirDate)
  {
    ...
    if (m_mediaType == MediaTypeEpisode)
    {
